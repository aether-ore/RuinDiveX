...
The V4 recovery status and the preserved historical 175-record backlog are maintained in [Dungeon Augmentation V4 Release-Issue Register](./DUNGEON_AUGMENTATION_V4_RELEASE_ISSUES.md). The canonical release witness now applies on its first realization with zero release-validation errors; the original records remain in that document as historical evidence.
