...
The V4 recovery status and the preserved historical 175-record backlog are maintained in [Dungeon Augmentation V4 Release-Issue Register](./DUNGEON_AUGMENTATION_V4_RELEASE_ISSUES.md). Seed001 has a current canonical attempt-one partial overlay with accepted release validation and zero errors. Seed000 has a current production attempt-one enclosure/recovery witness, but its canonical counts, hashes, validation receipt, and timings must be recaptured after the latest salvage and door-attribution integration. Release readiness remains **blocked** by the unchanged 30-second whole-seed performance gate, the repeated endpoint-seam provenance mismatch, and the still-pending current-hash visual and clean-source ordered release evidence. Historical failed results remain chronology only, not release-closure evidence.

## Current release readiness

- **Functionally passing Seed001:** status `applied` on outer realization attempt 1; 5 operations, 21 supplemental nodes, and 24 physical segments. Strict replay/progression/release validation passes with zero errors. The only whole-grant planner omission is `trapRoom_conveyorRoom`; exact conflict and dependency closure retain the safe two-node/two-segment `bossRoom_shrineRoom` remainder. Four later exact segment-5 replacements remove only colliding variants. Accepted hashes are `v1-603c269a7b1a5157aa66e1509fdf6b2a` / `v1-1aa997029ea651a640f47e7984073aad`.
- **Functionally passing Seed000, canonical recapture pending:** the current production `generate()` enclosure witness applies on realization attempt 1 and passes unchanged progression/enclosure assertions within its existing 180-second timeout after exact `Door_Shrine` recovery. The latest stored full diagnostic predates the current module-salvage and door-attribution integration: it completed in 166.4 seconds with 4 operations, 22 nodes, 27 segments, zero errors, 79/79 drop-space egress, and hashes `v1-f5178024e92254900ddaee251b78e15d` / `v1-76d7dfc517c1470584839152fcb8d227`. Those stored counts and hashes are chronology, not current-tree canonical evidence.
- **Performance failure:** the latest strict Seed001 probe completes functionally in 215.3 seconds wall time, with 35.6054 seconds of total planning, 35.1175 seconds of generator planning, and 211.581 ms of overlay materialization. The latest stored Seed000 diagnostic also failed the ceiling at 38.0677 seconds planning, but it predates the current salvage/door integration and must be recaptured. Both canonical seeds require current-tree evidence at or below the unchanged 30-second maximum before the ordered 10/100/1,000 tiers advance.
- **Focused contract evidence:** the planner/pruning/performance group passes 142/142, the broader augmentation group passes 159/159, and the combined lift/enclosure run passes 52/52, including all 4 dedicated automatic-lift geometry cases. The former `Door_Shrine` alternate route now produces an exact physical exclusion and same-seed modular recovery instead of surviving cached salvage. These are local contract checks, not formal visual or ordered release evidence.
...
- **Local browser observations, not formal visual receipts:** a pre-restart Seed001 diagnostic verified both authored ladder apertures plus all six automatic-lift boarding edges with zero blocking wall IDs. After the clean dev-server restart, the fresh strict page completed normally: the existing DOM readiness marker reached `true`, and a page-main-world inspection confirmed status `applied`, 5 operations, the accepted `v1-603c269a7b1a5157aa66e1509fdf6b2a` / `v1-1aa997029ea651a640f47e7984073aad` hashes, release validation accepted, and no console errors. The earlier `window.game === false` observation came from an isolated browser-evaluation world and was not an application publication failure. The lift screenshot and structured check at `artifacts/dungeon-augmentation-v4-release/visual/seed001-lift-aperture-after.png` and `artifacts/dungeon-augmentation-v4-release/visual/seed001-lift-aperture-after.json` remain supporting pre-restart diagnostics; a current-hash lift audit and formal two-seed visual receipt are still pending.
- **Auditable Seed000 chronology:** the latest stored accepted diagnostic is `artifacts/dungeon-augmentation-diagnostics/seed000-current-canonical-accepted-2026-08-02.json`; it predates the current module-salvage and door-attribution integration and must not be presented as a current-tree canonical receipt. The prior failed `artifacts/dungeon-augmentation-diagnostics/seed000-current-canonical-2026-08-02.json` remains before-fix chronology.
