...
This is the authoritative current working-tree checkpoint; older counts, timings, and hashes later in this file are investigation chronology, not current evidence.
...
| --- | --- | --- | --- |
| Automatic-lift boarding walls | **Repaired and live-evidenced** | The exact paired-lift aperture proof treated the lower stop's static support floor as a shaft blocker. A generic V4 wall-opening marker could hide that failure until floor metadata was merged or one lane disappeared, after which a boundary wall could be rebuilt in front of the lift. | Use the same lower-exclusive, upper-inclusive swept interval as lift movement; identify lift lanes from exact lift-edge metadata even if scalar zone metadata is merged; and forbid the generic V4 opening path from bypassing the complete paired six-lane proof. |
| Seed001 `trapRoom_conveyorRoom` omission | **Unresolved planner blocker** | Landmark candidate 13 (`stacked-interchange`) places pyramid segment 5 across endpoint 0 and exhausts that grant's endpoint domain. A bounded landmark-recovery replay is queued behind partial acceptance. A diagnostic run that forced the replay reached it but found no valid replacement, retained the same three-network result, and increased planning from about 36.3 seconds to 53.9 seconds; that slower no-op ordering was not retained. | Supply a legal landmark module/path replacement, or retain only parent-anchored safe fragments in a validated modular fallback; do not omit the entire later grant because of one landmark segment. |
| Seed001 `bossRoom_shrineRoom` omission | **Unresolved planner blocker** | All eight bounded dense identities fail. Six exhaust parent attachment; two fail authored elevation. The first required internal adjacency is 84 m while the featureless-span cap is 33.6 m, so all 240 isolated placement pairs are rejected. | Subdivide that exact adjacency within the existing six-module budget, or emit a validated parent-anchored forest that keeps safe endpoint modules and omits only the impossible internal span. |
| Seed001 `keycardRoom_trapRoom` omission | **Unresolved planner blocker** | All 12 bounded identities fail: 11 adjacent-domain exhaustion and one invalid ladder interval. The only approximately 33.6 m spine collides with authored `trapRoom_conveyorRoom_ground` occupied, camera, and column volumes. | Add a legal exterior/perpendicular reset path within the existing candidate window, or keep independently safe parent-anchored modules and omit only the conflicting internal entities. |
| Seed001 planning performance | **Unresolved release blocker** | Planning is about 36.3 seconds; materialization is only about 145 ms. The two proven-unbuildable full grants consume about 25.3 seconds, roughly 66% of candidate work. | Add sound structural preflight for the 84 m unsplittable adjacency and zero-compatible authored-volume collision, then use the modular fallback without increasing search limits. |
| Module-level salvage contract | **Designed, not implemented** | Solver and post-validation fallback can still represent failure only as a whole `prunedRouteNetworkGrant`; exact runtime conflicts are entity-attributed, but intrinsically unbuildable planner graphs have no reduced-graph representation. | Build and validate a renderer-free parent-anchored forest before hashing/materialization; ledger omitted nodes/segments canonically; require every retained component to connect bidirectionally to an exact parent socket; keep collision, seam, wall, lift/ladder, and platformability validators unchanged. |
...
| V4 authoritative automatic-lift boarding walls | **Seed001 live 12/12 edges across two lifts; 4/4 lift and 51/51 enclosure contracts evidenced; formal journey pending** | The paired aperture proof misclassified the lower stop's static sill/support floor as a shaft blocker. Generic V4 opening metadata could then bypass the failed exact proof until a merge or missing lane exposed a rebuilt wall. | Each lift still requires exact contiguous three-lane source and destination groups, opposing directions, matching centers and 14 m separation, reciprocal center links, and a clear lower-exclusive/upper-inclusive shaft. Exact lift-edge metadata suppresses the generic opening path even if scalar zone metadata was merged. Live Seed001 reports zero blockers on all 12 boarding edges. Focused artifacts are `artifacts/dungeon-augmentation-v4-release/visual/seed001-lift-aperture-after.png` and `.json`; they are not the formal receipt. |
...
- Seed000 full replay: **applied**, strict release validation accepted, realization attempt 1, 6 operations, 30 nodes, 37 segments, no pruned grants, no runtime repair, no errors, 79/79 drop-space lower tiles escapable, hashes `v1-c5bd067e2be80b0dab57afaf63f20d2b` / `v1-bc27dc53c2172c55eeb69fbafb6fc5db`, 25 seconds wall, 12.7841 seconds planning, 258.219 ms materialization.
- Seed001 full replay: **applied best-effort partial**, strict release validation accepted, realization attempt 1, 3 operations, 14 nodes, 17 segments, no runtime repair, no errors, hashes `v1-302ed26c9f2bc8dfdf8f598c28ac53c9` / `v1-4a48156df773fb747768ce9e95cb6f8a`, 45 seconds wall, 36.2653 seconds planning, 145.128 ms materialization. Planner omissions are exactly `keycardRoom_trapRoom`, `trapRoom_conveyorRoom`, and `bossRoom_shrineRoom`.
- Live Seed001 Playwright acceptance passes 1/1 and inventories two automatic lifts. Each endpoint has exactly three declared boarding lanes; all 12 floors resolve and every `blockingWallIds` array is empty.
- Dedicated automatic-lift geometry passes 4/4. The combined enclosure alias passes 51/51, including the production-parity nonzero-elevation witness.
- Planner scheduling/topology focus passes 69/69 after the full bounded-window and deterministic two-station candidate-order corrections. Seed000 retains identical hashes across the optimized plan-only and full replay probes.
- The focused lift artifacts remain `artifacts/dungeon-augmentation-v4-release/visual/seed001-lift-aperture-after.png` and `seed001-lift-aperture-after.json`. They cover one lift and are supporting diagnostics, not a sealed formal receipt; the live acceptance manifest is the current two-lift proof.
- The formal two-seed receipt, canonical -> 10 -> 100 -> 1,000 ordered evidence, clean-source receipts, release provenance, and sealed attestation remain pending. Seed001's planning time and whole-grant fallback behavior block advancing those gates.
